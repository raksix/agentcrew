...
export function SubagentWindow({ subagent, onUpdate }: SubagentWindowProps) {
  ...
  return (
    <div className={`${config.bg} border border-transparent hover:border-gray-200 dark:hover:border-gray-700 rounded-xl p-4`}>
      ...
      {subagent.status === 'running' && (
        <div className="mt-3 h-1 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
          <div className="h-full bg-yellow-500 animate-pulse w-1/2" />
        </div>
      )}
    </div>
  );
